...
export default function NewBookPageClient() {
    ...
    // URLパラメータからの初期データをチェック
    const hasUrlParams = searchParams.has("title");
    const isManualMode = searchParams.get("manual") === "true";

    // フォームデータ（URLパラメータがあれば初期値として使用）
    const [formData, setFormData] = useState<BookFormData>({
        title: searchParams.get("title") || "",
        author: searchParams.get("author") || "",
        tags: searchParams.get("categories")?.replace(/,/g, ", ") || "",
        coverImageUrl: searchParams.get("coverUrl") || null,
        googleBooksId: searchParams.get("googleBooksId") || null,
        pageCount: searchParams.get("pageCount") ? parseInt(searchParams.get("pageCount")!) : null,
        publishedDate: searchParams.get("publishedDate") || null,
        description: searchParams.get("description") || null,
    });

    // 検索ステップをスキップするかどうか
    const skipSearchStep = hasUrlParams || isManualMode;
        ...
        <div className="galaxy-bg min-h-screen">
            ...
            <main className="pt-24 px-6 max-w-3xl mx-auto pb-12">
                ...
                <div className="space-y-6">
                    ...
                    {!skipSearchStep && (
                        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6">
                            ...
                            <p className="mt-4 text-sm text-white/50 text-center">
                                または{" "}
                                <Link href="/books/search" className="text-purple-400 hover:text-purple-300">
                                    検索画面で探す →
                                </Link>
                            </p>
                        </div>
                    ...
                    {/* 検索画面から来た場合のメッセージ */}
                    {skipSearchStep && formData.title && (
                        <div className="bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-purple-500/20 rounded-2xl p-4 flex items-center gap-4">
                            {formData.coverImageUrl && (
                                <img src={formData.coverImageUrl} alt="" className="w-16 h-24 object-cover rounded-lg" />
                            )}
                            <div>
                                <p className="text-purple-300 text-sm mb-1">選択した本:</p>
                                <p className="text-white font-semibold">{formData.title}</p>
                                {formData.author && <p className="text-white/60 text-sm">{formData.author}</p>}
                            </div>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6">
                            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                                <span className="w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-sm">{skipSearchStep ? "1" : "2"}</span>
                                本の情報
                            </h3>
                        ...
                        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6">
                            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                                <span className="w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-sm">{skipSearchStep ? "2" : "3"}</span>
                                星のイメージを設定
                            </h3>
